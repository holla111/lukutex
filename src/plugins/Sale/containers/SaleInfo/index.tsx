...
export const SaleInfo: React.FC<SaleInfoProps> = (props: SaleInfoProps) => {
    ...
    const dispatchGetTotalBuyers = (ieoID) => dispatch(getTotalBuyers({
        ieo_id: ieoID
    }))
            ...
            <div className="row">
                <div className="col-12 text-center">
                    <Row gutter={[16, 16]}>
                        ...
                        <Col span={8}>
                            <Statistic valueStyle={{ color: '#e76f51', fontWeight: 'bold' }} title="Total Buyers" value={totalBuyersSelector.payload.totalBuyers} />
                        </Col>
                        <Col span={8}>
                            <Statistic valueStyle={{ color: '#f4a261' }} title={"Total"} value={props.sale.total_ieo} />                        </Col>
                    </Row>
                    <hr />
                    <ProgressBar animated variant="info" now={(props.sale.total_ieo - props.sale.remains) / props.sale.total_ieo * 100} label="" style={{ height: '40px', fontSize: '1rem', fontWeight: 'bold' }} />
                </div>
            </div>
