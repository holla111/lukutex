...
class LunarGame extends React.Component<LunarGameProps, LunarGameState> {
  ...
  public boxsRender = () => {
    ...
    const handleClickBox = (index:number) => {
      ...
      user.uid && txid && this.props.rewardPost({
  ...
  public playGame = () => {
    ...
    if (this.scrollTopElm) { this.scrollTopElm.scrollIntoView({ behavior: 'smooth' }); }
    ...
    // tslint:disable-next-line: ban
    this.animationTimeOut.forEach(value => { clearTimeout(value); });//clear animation - reset - handle the user's click spam
    this.animationTimeOut[0] = setTimeout(() => {
      this.myTween = this.boxElements.map((element, i) => TweenLite.to(element, 0.5, { x: arrPositionX[i], y: arrPositionY[i], ease: 'elastic.out(0.2, 0.1)' }));
    }, 0.5 * 4 * 1000);
    this.animationTimeOut[1] = setTimeout(() => {
      this.myTween = this.boxElements.map(element => TweenLite.to(element, 0.5, { x: 0, y: 0, ease: 'elastic.out(0.2, 0.1)' }));
    }, 0.5 * 5 * 1000);
  };
  ...
  public render() {
    ...
    return (
      <div className="container-fluid pg-lunar-game" style={{ backgroundImage: `url(${bgImg})` }}>
        ...
        {this.state.isShowResultModal && <Confetti
...
const mapDispatchProps: MapDispatchToPropsFunction<DispatchProps, {}> = dispatch => ({
  ...
  rewardPost: ({uid,txid,cb}:{uid:string,txid:string,cb:(payload : RewardsDataResponse) => void}) => dispatch(rewardPost({uid,txid,cb})),
});
