...
export const ClaimTab: React.FC<ClaimTabProps> = (props: ClaimTabProps) => {
    ...
    const handleClaimButton = () => {
        ...
        axios.get(`/claim/find/airdrop_id=${props.airdropID}&user_ip=${ipV4}/`)
            .then(response => {
                const ipList = response.data.payload;
                if (ipList.length > 0) {
                    axios.post('/block/' + claimData.user_uid)
                        .then(response => {
                    ...
                    axios.post('/claim/airdrop_id=' + props.airdropID, claimData)
                        .then(response => {
                            switch (response.data.msg) {
                                case 'out_of_tokens':
                                    ...
                                    break;
                                case 'exist_user_uid':
                                    message.error('Hmmm... Maybe you has claimed yet. 1 account just receives one. Sorry for that.');
                                    break;
                                case 'claim_success':
    ...
    let claimResult = <Result
        icon={<SmileOutlined />}
        title="Great, we have done all the tasks!"
        extra={<Button disabled={ipV4 === '' || claimClickState} type="primary" onClick={handleClaimButton}>{ipV4 === '' ? 'Waiting ...' : 'Claim'}</Button>}
    />;
    if (claim && claim.payload.length > 0) {
        switch (claim.payload[0].claim_status) {
            case 0:
                claimResult = <Result
                    icon={<SmileOutlined />}
                    title="Thanks you for your participating!"
                />;
                break;
            case 1:
                claimResult = <Result
                    status="success"
                    title="Thanks, you are eligible for receive tokens. Remember follow us at LuKuTex channel on Twitter."
                />;
                break;
            case 2:
                claimResult = <Result
