...
export const FacebookTab: React.FC<FacebookTabProps> = (props: FacebookTabProps) => {
    ...
    const handleResponse = (data) => {
        if (!facebookTaskState.sharePage) message.error('You have\'t share page task 2.');
        if (!facebookTaskState.likePost) message.error('You have\'t like post task 3.');
        if (!facebookTaskState.sharePost) message.error('You have\'t share post task 4.');
        ...
        <a onClick={() => setFacebookState({ ...facebookTaskState, sharePage: true })}>
            <FacebookProvider appId={props.facebookConfig.facebookAppId}>
                <Share href={props.facebookConfig.facebookPage}>
                    {({ handleClick, loading }) => (
                        <Button type="primary" onClick={handleClick}>Share</Button>
                    )}
                </Share>
            </FacebookProvider>
        </a>
    ...
    const likePostButton =
        <Button onClick={() => setFacebookState({ ...facebookTaskState, likePost: true })} type="primary" href={props.facebookConfig.facebookPost} target="_blank">Like</Button>

    const sharePostButton =
        <a onClick={() => setFacebookState({ ...facebookTaskState, sharePost: true })}><FacebookProvider appId={props.facebookConfig.facebookAppId}>
    ...
    return (
        <div>
            { loginState.status && facebookTaskState.sharePage && facebookTaskState.likePost && facebookTaskState.sharePost ? resultView : tableView}
        </div>
    )
}
