...
export const TwitterTab: React.FC<TwitterTabProps> = (props: TwitterTabProps) => {
    ...
    const handleUserNameSubmitClick = () => {
        ...
        api.get(`/claim/find/airdrop_id=${props.airdropID}&twitter_username=${userNameInputState}`)
        .then(response => {
    ...
    const dataSource = [
        {
            key: '1',
            step: '1',
            task: twitterUsername === '' ? `Enter Twitter username. Ex: @${props.twitterConfig.twitterUserName}` : '@' + twitterUsername,
        ...
        },
        {
            ...
            ? <Button type="primary" disabled={followState} onClick={() => handleFollowCheckButton()} >Verify</Button> 
            : <Button onClick={() => setFolowClickState(true)} type="primary" href={props.twitterConfig.twitterPage} target="_blank" disabled={twitterUsername === ''}>Follow</Button>,
        },
        {
            key: '3',
            step: '3',
            ...
            ? <Button type="primary" onClick={() => handleTweetCheckButton()} >Verify</Button> 
            : <Button onClick={() => setTweetClickState(true)} type="primary" href={props.twitterConfig.twitterPost} target="_blank" disabled={twitterUsername === '' || !followState || tweetState}>Tweet</Button>,
        },
    ...
    return (
        <div>
            {followState && tweetState ? resultView : tableView}
        </div>
    )
}
