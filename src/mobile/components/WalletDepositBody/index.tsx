...
const WalletDepositBodyComponent = (props) => {
  ...
  const renderDeposit = (isAccountActivated: boolean) => {
    ...
    const textMinDeposit = `${translate('page.body.wallets.tabs.deposit.ccy.message.mindeposit')} ${
      Number(currencyItem.min_deposit_amount) + Number(currencyItem.deposit_fee)
    ...
    if (wallet.type === 'coin') {
      return (
        <React.Fragment>
          ...
          {currencyItem && !currencyItem.deposit_enabled ? (
            <Blur className={blurCryptoClassName} text={translate('page.body.wallets.tabs.deposit.disabled.message')} />
          ) : null}
      ...
      return (
        <React.Fragment>
          <CurrencyInfo wallet={wallet} />
          {currencyItem && !currencyItem.deposit_enabled ? (
            <Blur className="pg-blur-deposit-fiat" text={translate('page.body.wallets.tabs.deposit.disabled.message')} />
          ) : null}
